...
namespace HomeLibrary.WebForms.Helpers
{
    /// <summary>
    ...
    {
        private static readonly Regex PageTailRegex = new Regex(
            @"[—\-–]?\s*(?:с|стр)\.?\s*(\d+)\s*$",
        ...
        private static void AppendEntry(StringBuilder sb, XElement element)
        {
            ...
            sb.Append(WebUtility.HtmlEncode(title != null ? title.Trim() : string.Empty));
        ...
        public static string NormalizeForStorage(string editorHtml)
        {
            ...
            string normalized;
            if (TryParseXml(wrapped, out normalized))
            {
                return normalized;
            }
